    ...
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                ...
                listTypes = [];
                if (!(relatedFields.length === 0)) return [3 /*break*/, 2];
                return [4 /*yield*/, (0, writeFile_1.writeFile)(path_1.default.join(process.cwd(), "prisma/generated/models/".concat(model.name.toLowerCase(), "/dataLoaders.ts")), "const ".concat((0, capitalize_1.unCapitalize)(model.name), "Loader = () => {}"))];
            ...
            case 2:
                resolverFile = "\n    import { default as DataLoader } from 'dataloader';\n    import { ".concat(relatedFields
                    ...
                    .filter(function (value, index, array) { return array.indexOf(value) === index; }), ", ").concat(model.name, "} from '@prisma/client'\n    import { getDB } from '@/db';\n\n    // Add base model loader\n    const ").concat((0, capitalize_1.unCapitalize)(model.name), "Loader = () => \n      async (ids: readonly string[]): Promise<(").concat(model.name, " | undefined)[]> => {\n        const db = await getDB();\n        const items = await db.").concat((0, capitalize_1.unCapitalize)(model.name), ".findMany({\n          where: {\n            id: { in: [...ids] }\n          }\n        });\n        return ids.map(id => items.find(item => item.id === id));\n      };\n\n    ").concat(relatedFields
                    .map(function (rf) {
                    var _a, _b;
                    if (rf.isArray) {
